...
all_num = subset(airbnb_hot_encoding, select = -c(amenities,host_location,host_is_superhost,host_has_profile_pic,host_identity_verified,neighbourhood_cleansed,property_type,room_type,instant_bookable,bathroom_pri_shared))
...
#combine all_num with amenities 
airbnb_all_features = cbind(all_num, amenities_table_price = subset(amenities_table, select = -c(price,id)))
